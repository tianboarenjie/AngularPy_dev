...
from .inventory import ApyInventory
...
class PlayBookRunner(object):
    """
    用于执行AnsiblePlaybook接口，简化Playbook对象使用
    """
    Options = namedtuple("Options", [
        "listtags", "listtasks", "listhosts", "syntax", "connection", "module_path",
        "forks", "remote_user", "private_key_file", "timeout", "ssh_common_args", "ssh_extra_args",
    ...
    def __init__(self,hosts=None,
        ...
        if playbook_path is None or not os.path.exists(playbook_path):
            raise EnvironmentError("Not Found the playbook file: %s." % playbook_path)
        self.playbook_path = playbook_path
        self.loader = DataLoader()
        self.variable_manager = VariableManager()
        self.passwords = passwords or {}
        self.inventory = ApyInventory(hosts)
    ...
    def run(self):
        if not self.inventory.list_hosts("all"):
            raise EnvironmentError("Inventory is empty")
        self.runner.run()
...
class AdHocRunner(object):
    ...
    """
    Options = namedtuple("Options", [
        "connection", "module_path", "private_key_file", "remote_user", "timeout", "forks", "become",
        "become_method", "become_user", "check", "extra_vars"
    ])
    results_callback_class = AdHocResultCallback

    def __init__(self, hosts=C.DEFAULT_HOST_LIST,
        ...
        self.variable_manager.options_vars = load_options_vars(self.options)
        self.passwords = passwords or {}
        self.inventory = ApyInventory(host_list=hosts)
        self.variable_manager.set_inventory(self.inventory)
        self.tasks = []
    ...
    def run(self, task_tuple, pattern="all", task_name="Ansible Ad-hoc"):
        """
        ansible run
        :param task_tuple: (("shell", "ls -lhtr /"), ("ping",""))
        :param pattern:
        :param task_name:
        :return:
        """
        for module_name, args in task_tuple:
            if not self.check_module_args(module_name, args):
                return
            self.tasks.append(
                dict(action=dict(
                    module=module_name,
        ...
        if not self.inventory.list_hosts(self.patten):
            raise EnvironmentError("pattern: %s does not match any hosts." % self.patten)
        ...
        try:
            ...
            return self.results_callback.result_query
        finally:
            if self.runner:
                self.runner.cleanup()
            if self.loader:
    ...
    def clean_result(self):
        ...
        for host in self.results_callback.result_query["contacted"]:
            result["success"].append(host)

        for host, msgs in self.results_callback.result_query["dark"].items():
            msg = "\n".join(["{} {}: {}".format(
...
def test_run():
    ...
    task_tuple = (("command", "hostname"), ("setup", ""))
    # task_tuple = (("apt", "name=htop state=absent"),)
    hoc = AdHocRunner(assets)
    hoc.results_callback = AdHocResultCallback()
    ret = hoc.run(task_tuple)
    print(ret)

    # play = PlayBookRunner(assets, playbook_path="test.yml")
    # ret = play.run()
    # print(ret)
